  ...
  return (
    ...
    >
      <div className="h-48 md:h-64 relative overflow-hidden shrink-0">
        ...
        <div className="absolute top-4 right-4 bg-[var(--bg)]/60 backdrop-blur-md px-3 py-1 rounded-full flex items-center gap-1 text-[var(--accent)] text-sm font-bold shadow-sm">
          <Star size={14} fill="currentColor" />
          {rating} <span className="text-[var(--text-muted)] text-xs font-normal">({reviews})</span>
        </div>
        <div className="absolute bottom-4 left-4 bg-[var(--bg)]/60 backdrop-blur-md px-3 py-1 rounded-full flex items-center gap-1 text-[var(--text)] text-xs font-bold shadow-sm">
          <MapPin size={12} className="text-[var(--accent)]" />
          {city}
        </div>
        {isVerified && (
          <div className="absolute top-4 left-4 bg-[var(--success)] text-white p-1.5 rounded-full shadow-lg">
            <ShieldCheck size={16} />
          </div>
        )}
      </div>
      <div className="p-4 md:p-6 flex flex-col flex-1">
        ...
        <div className="flex gap-2 mt-auto">
          <button 
            onClick={() => {
              onAction?.(`Viewing profile of ${name}`);
            }}
            className="flex-1 bg-[var(--text)]/5 border border-[var(--border)] text-[var(--text)] rounded-xl md:rounded-2xl font-bold text-xs md:text-sm hover:bg-[var(--text)]/10 transition-colors active:scale-95 py-2.5 md:py-3.5 flex items-center justify-center gap-2"
          >
            <User size={16} />
            View Profile
          </button>
          <button 
            onClick={() => {
              onAction?.(`Starting booking process with ${name}`);
            }}
            className="flex-1 bg-[var(--accent)] text-[var(--accent-foreground)] py-2.5 md:py-3.5 rounded-xl md:rounded-2xl font-bold text-xs md:text-sm hover:opacity-90 transition-colors active:scale-95 shadow-lg shadow-[var(--accent)]/20 flex items-center justify-center gap-2"
          >
            <Calendar size={16} />
            Book Now
          </button>
