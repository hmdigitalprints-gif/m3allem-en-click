...
export default function PaymentModal({ booking, onClose, onSuccess, onAction }: PaymentModalProps) {
  const [method, setMethod] = useState<'card' | 'wallet' | 'cash' | 'm-pesa'>('card');
          ...
          <div className="p-8 md:p-12">
            ...
            <div className="space-y-4 mb-8">
              ...
              <button 
                ...
                className={`w-full p-6 rounded-2xl border-2 transition-all flex items-center gap-4 ${method === 'm-pesa' ? 'border-[var(--accent)] bg-[var(--accent)]/5' : 'border-[var(--border)] hover:border-[var(--accent)]/30'}`}
              >
                <div className={`p-3 rounded-xl ${method === 'm-pesa' ? 'bg-[var(--accent)] text-white' : 'bg-[var(--card-bg)] text-[var(--text-muted)]'}`}>
                  <Wallet size={24} />
                </div>
                <div className="text-left">
                  <p className="font-bold">Mobile Money</p>
                  <p className="text-xs text-[var(--text-muted)]">Orange Money, Inwi Money, M-Pesa</p>
                </div>
              </button>
