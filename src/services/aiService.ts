...
export async function getAutoEstimate(serviceType: string, description: string, city: string, urgency: string): Promise<any> {
  try {
    const ai = getAi();
    const prompt = `Provide a detailed price estimate for: ${description} in category ${serviceType}. Urgency: ${urgency}, City: ${city}. Return JSON with suggested (number), minPrice, maxPrice, and breakdown (array of strings).`;
    const response = await ai.models.generateContent({
      model: "gemini-3-flash-preview",
      contents: prompt,
      config: { responseMimeType: "application/json" }
    ...
    return { suggested: 200, minPrice: 150, maxPrice: 300, breakdown: ["Standard labor", "Basic materials"] };
  }
}
