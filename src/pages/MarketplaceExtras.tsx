...
export default function MarketplaceExtras() {
    ...
    const fetchData = async () => {
      try {
        const [pkgRes, grpRes] = await Promise.all([
          fetch('/api/marketplace-extras/packages', { credentials: 'include' }),
          fetch('/api/marketplace-extras/group-requests', { credentials: 'include' })
        ]);
  ...
  return (
    <Layout>
      ...
      {showBookingModal && selectedPackage && (
        <BookingModal 
          ...
          onSuccess={() => {
            ...
            setToastMessage("Package request submitted successfully!");
            setTimeout(() => setToastMessage(null), 3000);
          }}
        />
      )}
      ...
      {toastMessage && (
        <div className="fixed bottom-32 left-1/2 -translate-x-1/2 z-[100] bg-[var(--card-bg)] text-[var(--text)] px-6 py-3 rounded-2xl font-bold shadow-2xl flex items-center gap-3 border border-[var(--border)]">
