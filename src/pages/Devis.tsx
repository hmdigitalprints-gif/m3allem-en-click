        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
          <div>
            <h1 className="text-4xl md:text-6xl font-bold tracking-tighter mb-4 text-[var(--text)]">
              Your <span className="text-[var(--accent)]">Estimates.</span>
            </h1>
            <p className="text-[var(--text-muted)] text-lg">Manage and review your project quotes.</p>
          </div>
          <div className="flex flex-col sm:flex-row gap-4">
            ...
            >
              <Sparkles size={20} className="text-[var(--accent)]" /> AI Automatic Quote
            </Link>
            <button className="bg-[var(--accent)] text-[var(--accent-foreground)] px-8 py-4 rounded-full font-bold hover:opacity-90 transition-colors flex items-center gap-3 shadow-xl shadow-[var(--accent)]/20">
              <Plus size={20} /> Request New Estimate
            </button>
          </div>
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-[var(--border)] text-[var(--text-muted)] text-xs uppercase tracking-widest">
                  ...
                  <th className="p-6 font-bold">Artisan</th>
              ...
              <tbody className="divide-y divide-[var(--border)]">
                {[1, 2, 3, 4]?.map((i) => (
                  <tr key={i} className="hover:bg-[var(--bg)] transition-colors group">
                    ...
                    <td className="p-6 font-bold text-[var(--text)]">Complete Bathroom Renovation</td>
                    <td className="p-6 text-[var(--text)]/80">Master Plumber</td>
