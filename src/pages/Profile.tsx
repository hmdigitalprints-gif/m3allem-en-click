...
import { User, Settings, CreditCard, ShieldCheck, LogOut, ChevronRight, Sparkles, Globe } from 'lucide-react';
...
export default function Profile() {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await updateProfile({ name: editName, avatarUrl: editAvatar || undefined });
      setIsEditing(false);
    } catch (err) {
      console.error(err);
    } finally {
    ...
    <Layout>
      <div className="p-6 md:p-12 max-w-4xl mx-auto">
        <div className="mb-12">
          ...
          <p className="text-[var(--text-muted)] text-lg">Manage your account and preferences.</p>
        </div>
          ...
          <div className="flex-1 text-center md:text-left">
            {isEditing ? (
              ...
              <>
                ...
                <p className="text-[var(--text-muted)] mb-4">{user?.phone || ''}</p>
