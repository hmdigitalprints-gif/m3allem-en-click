      ...
      <div className="p-6 md:p-12 max-w-4xl mx-auto">
        ...
        <div className="bg-[var(--card-bg)] border border-[var(--border)] rounded-[40px] p-8 md:p-12 mb-8 flex flex-col md:flex-row items-center gap-8">
          <div className="relative group cursor-pointer">
            ...
            <label className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 rounded-full cursor-pointer opacity-0 group-hover:opacity-100 transition-opacity">
              <Settings size={24} className="text-white mb-1" />
        ...
        <div className="space-y-4">
          {[
            ...
            <button 
              ...
              className="w-full bg-[var(--card-bg)] hover:bg-[var(--bg)] border border-[var(--border)] rounded-3xl p-6 flex items-center gap-6 transition-colors group text-left cursor-pointer"
            >
        ...
        <button 
          onClick={logout}
          className="w-full mt-12 bg-[var(--destructive)]/10 hover:bg-[var(--destructive)]/20 text-[var(--destructive)] border border-[var(--destructive)]/20 rounded-3xl py-5 font-bold flex items-center justify-center gap-3 transition-colors cursor-pointer"
        >
        ...
        {activeModal && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md">
            <motion.div
              ...
              className="bg-[var(--card-bg)] border border-[var(--border)] rounded-[40px] p-8 w-full max-w-md relative overflow-hidden text-center shadow-2xl"
            >
              ...
              <div className="w-20 h-20 rounded-3xl bg-[var(--accent)]/10 flex items-center justify-center text-[var(--accent)] mx-auto mb-6">
                <Sparkles size={40} />
              </div>
              <h2 className="text-2xl font-bold mb-3 text-[var(--text)]">{activeModal}</h2>
              <p className="text-[var(--text-muted)] mb-8">
                This feature is currently under active development. You will be able to manage your {activeModal.toLowerCase()} soon.
              </p>
              <button 
                onClick={() => setActiveModal(null)}
                className="w-full bg-[var(--bg)] border border-[var(--border)] text-[var(--text)] hover:text-[var(--accent)] hover:border-[var(--accent)]/50 py-4 rounded-2xl font-bold transition-all cursor-pointer"
              >
                Got it, thanks!
              </button>
            </motion.div>
