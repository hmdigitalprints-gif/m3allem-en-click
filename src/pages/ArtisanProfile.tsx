    ...
    return (
      <Layout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center bg-[var(--bg)]">
          <h2 className="text-2xl font-bold mb-4 text-[var(--text)]">{t('artisan_not_found', 'Artisan not found')}</h2>
          <Link to="/find-pro" className="text-[var(--accent)] hover:underline">{t('return_to_search', 'Return to search')}</Link>
        </div>
      </Layout>
    );
  }
    ...
    <Layout>
      <div className="p-6 md:p-12 max-w-4xl mx-auto bg-[var(--bg)]">
        <Link to="/find-pro" className="inline-flex items-center gap-2 text-[var(--text-muted)] hover:text-[var(--text)] transition-colors mb-8">
          <SymmetricalIcon icon={ArrowLeft} size={20} /> {t('back_to_search', 'Back to Search')}
        </Link>
        ...
        <div className="bg-[var(--card-bg)] border border-[var(--border)] rounded-[40px] p-8 md:p-12 flex flex-col md:flex-row gap-12 items-start relative overflow-hidden mb-12 shadow-2xl">
          <div className="absolute top-0 end-0 w-64 h-64 bg-[var(--accent)]/5 rounded-full blur-3xl -z-10 translate-x-1/2 -translate-y-1/2"></div>
          ...
          <div className="w-32 h-32 md:w-48 md:h-48 rounded-full bg-[var(--bg)] shrink-0 border-4 border-[var(--border)] shadow-2xl relative">
            ...
            {artisan.is_online && (
              <div className="absolute bottom-2 end-2 w-6 h-6 bg-[var(--success)] rounded-full border-4 border-[var(--bg)]"></div>
            )}
          </div>
          ...
          <div className="flex-1 text-start">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-6">
              <div>
                ...
                <p className="text-[var(--accent)] text-xl font-medium flex items-center gap-2">
                  {t(artisan.category_name, artisan.category_name)} {artisan.is_verified && <ShieldCheck size={20} />}
                </p>
              </div>
              <div className="flex items-center gap-4">
                ...
                <span className="text-[var(--text-muted)] text-sm">({t('reviews_count', { count: artisan.review_count })})</span>
              </div>
            </div>

            <div className="flex items-center gap-6 text-[var(--text-muted)] mb-8">
              <div className="flex items-center gap-2"><MapPin size={18} className="text-[var(--accent)]" /> {t(artisan.city || 'Casablanca', artisan.city || 'Casablanca')}</div>
              <div className="flex items-center gap-2"><Calendar size={18} className="text-[var(--accent)]" /> {t('years_experience_val', { years: artisan.years_experience })}</div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <button className="flex-1 bg-[var(--accent)] text-[var(--accent-foreground)] px-8 py-4 rounded-2xl font-bold text-lg hover:bg-[var(--accent)]/90 transition-colors flex items-center justify-center gap-3">
                <Calendar size={20} /> {t('book_now_btn', 'Book Now')}
              </button>
              <button 
              ...
              >
                <Video size={20} /> {t('live_diagnostic_btn', 'Live Diagnostic')}
              </button>
              <button className="flex-1 bg-[var(--bg)] border border-[var(--border)] text-[var(--text)] px-8 py-4 rounded-2xl font-bold text-lg hover:bg-[var(--bg)]/80 transition-all flex items-center justify-center gap-3">
                <MessageSquare size={20} /> {t('send_message_btn', 'Send Message')}
              </button>
            </div>
          </div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12 text-start">
                ...
                <div className="flex flex-col gap-3">
                  {artisan?.certifications?.split(',')?.map((cert, i) => (
                    <div key={i} className="flex items-start gap-3 text-[var(--text-muted)] text-sm">
                      ...
                      <span>{t(cert.trim(), cert.trim())}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div className="md:col-span-2">
            <h3 className="text-2xl font-bold mb-6 text-[var(--text)]">{t('portfolio_title', 'Portfolio')}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              {artisan?.portfolio?.map((item) => (
                <div key={item.id} className="group bg-[var(--card-bg)] border border-[var(--border)] rounded-3xl overflow-hidden shadow-xl text-start">
                  ...
                  <div className="p-6">
                    <h4 className="font-bold mb-2 text-[var(--text)]">{t(item.title, item.title)}</h4>
                    <p className="text-[var(--text-muted)] text-sm">{t(item.description, item.description)}</p>
                  </div>
                </div>
              ))}
              {artisan.portfolio.length === 0 && (
                <div className="col-span-2 text-center py-12 text-[var(--text-muted)] bg-[var(--card-bg)] border border-[var(--border)] rounded-3xl">
                  {t('no_portfolio_items', 'No portfolio items available yet.')}
                </div>
              )}
            </div>
          </div>
        ...
        <div className="mt-12 text-start">
          <div className="flex items-center justify-between mb-8">
            <h3 className="text-3xl font-bold tracking-tighter text-[var(--text)]">
              {t('reviews_title_1', 'Client')} <span className="text-[var(--accent)]">{t('reviews_title_2', 'Reviews.')}</span>
            </h3>
          ...
          <div className="space-y-6">
            ...
            {(!artisan.reviews || artisan.reviews.length === 0) && (
              <div className="text-center py-16 bg-[var(--card-bg)] border border-[var(--border)] rounded-[40px]">
                <p className="text-[var(--text-muted)] text-lg">{t('no_reviews_yet', 'No reviews yet. Be the first to book and leave a review!')}</p>
              </div>
            )}
          </div>
        </div>
